...
# Setup Nattka
nattka_bugzilla = NattkaBugzilla(api_url="{0}/rest".format(bugzilla_url), api_key=bugzilla_api_key)
...
def oneshot_msg(bug, num, message):
        # Send a one-off message to IRC via Irker
        identifier = "{0}-{1}".format(arch, machine)
        atoms = ""
        if bug:
                atoms = " ,".join(bug.atoms.split("\r\n")[:2])

        message = "[{0}] [bug #{1} ({2})] {3}".format(identifier, num, atoms, message)
...
def reserve_bug(num):
        ...
        bzapi = bugzilla.Bugzilla("{0}/xmlrpc.cgi".format(bugzilla_url), api_key=bugzilla_api_key)
...
def start_working(bug, num):
        ...
        # Run useflags.sh
        oneshot_msg(bug, num, "running useflags.sh")
        process = subprocess.run("./{0}-useflags.sh".format(tatt_base), stdout=subprocess.DEVNULL)

        # Sometimes run rdeps (they don't always exist)
        rdeps_path = "{0}-rdeps.sh".format(tatt_base)
        if os.path.isfile(rdeps_path):
                oneshot_msg(bug, num, "running rdeps.sh")
                process = subprocess.run("./{0}".format(rdeps_path))
        else:
                oneshot_msg(bug, num, "no rdeps.sh here")
        ...
        with open(report_path, "r") as report:
                for line in report.readlines():
                        ...
                        if "merging test dependencies" in line:
                                print("[bug #{0}] failed to merge test dependencies in {1} phase".format(num, part))
                                results[part]["test_dep_failure"] += 1
                        elif "slot conflict" in line:
                                print("[bug #{0}] hit a slot conflict in {1} phase".format(num, part))
                                results[part]["slot_conflict"] += 1
                        elif "blocked" in line:
                                print("[bug #{0}] hit a blocker in {1} phase".format(num, part))
                                results[part]["blocked"] += 1
                        elif "failed" in line:
                                print("[bug #{0}] failed for unknown reasons in {1} phase".format(num, part))
                ...
                for part in ["USE", "revdep"]:
                        ...
                        summary = "[{0}] succeeded: {1}, test dep fail: {2}, slot conflict: {3}, blocked: {4}, unknown: {5}".format(
                                part, success, test_dep_failure, slot_conflict, blocked, unknown_failure
                        )

                        print("[bug #{0}] {1}".format(num, summary))
                        oneshot_msg(bug, num, summary)
...
def worker_loop():
        for num,bug in bugs.items():
                ...
                process = subprocess.run("tatt -b {0} -j {0}-{1}-{2}".format(num, arch, machine).split(), stdout=subprocess.DEVNULL)
...
try:
        ...
        oneshot_msg(None, "-1", "croaking due to exception on {0}-{1}".format(arch, machine))
        raise e
